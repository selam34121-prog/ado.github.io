...
const authChanged=()=>window.dispatchEvent(new Event('ado:auth-changed'));
export async function register(email,password,inviteCode){const data=await request('/auth/register',{method:'POST',body:JSON.stringify({email,password,inviteCode})});settings.set('syncToken',data.token);settings.set('syncEmail',data.email);authChanged();await pushCloud()}
export async function login(email,password){const data=await request('/auth/login',{method:'POST',body:JSON.stringify({email,password})});settings.set('syncToken',data.token);settings.set('syncEmail',data.email);authChanged();await pullCloud(true)}
export function logout(){settings.set('syncToken','');settings.set('syncEmail','');settings.set('syncUpdatedAt',0);authChanged()}
